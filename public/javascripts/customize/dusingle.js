window.onload = function () {
    ...
    console.log("onload");
    $(".selectpicker").selectpicker('val', '默认相册');
...
var inputFileEvent = function () {
    ...
    if (this.files.length !== 0) {
        ...
        if (numOfImages < 5) {
            ...
            $(newImgForm).find(".selectpicker").selectpicker('val', '默认相册');
            this.parentElement.getElementsByTagName("img")[0].src = url;
            ...
            $(newImgForm).find("input.watermark").on("click", watermarkEvent);
...
var postData = function () {
    ...
    $.ajax({
        ...
        error: function (data) {
            console.log(data);
            $("#progressModal").modal("hide"); // 隐藏进度条
            $("#snackbar").text("上传失败").addClass("show");
            setTimeout(function () {
                $("#snackbar").text("消息提示条").removeClass("show");
                window.location.href = '/upload/images';
            }, 2000);
